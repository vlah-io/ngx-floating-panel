...
const directoryPath = path.join(__dirname, 'projects/ngx-floating-panel/src/lib');
const results = [];
...
try {
  ...
  const fp = fs.createWriteStream(path.join(__dirname, 'projects/ngx-floating-panel/src/public-api.ts'), {
    flags: 'w'
  });
  results.forEach((str) => {
