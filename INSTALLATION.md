...
```
  ng new ngx-floating-panel --createApplication=false --prefix=vlah
  cd ngx-floating-panel
```
...
```
  cd ngx-floating-panel
  ng generate library ngx-floating-panel --prefix=vlah
```
...
```
  cd ngx-floating-panel
  ng generate application ngx-floating-panel-test
```
...
4. When using directives depended on `CSS` in the `test` application don't forget to add the necessary imports (see the example bellow).
   ```
    @import "../../ngx-floating-panel/src/assets/css/ngx-floating-panel.css";
   ```
